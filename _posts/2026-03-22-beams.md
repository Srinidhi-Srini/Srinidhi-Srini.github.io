...
## Example
It's hard to still understand it through just reading it, so I will do a super simple example to it. I'm not going to do method 1 since it takes so much more time than method 2, and method 2 is the much easier and better method. 
...
Next step is to section off the beam to take the integrals. The logical step would to make $$0<x<2$$ Section 1, and $$2<x<4$$ Section 2 since the distributed force ends at 2 m. 
...
Now, let's do section 2, which is from $$ 2 < x < 4$$.
> $$V_{2} = V_{A} - \int_{0}^{x} W(x) \,dx $$
> 
> First, our $$V_{A}$$ will be $$B_{y} = -80 N$$, as that's the external force present at the beginning of the section. Next, $$W(x)$$ is the equation of the disributed force. Since there is no distributed force from $$ 2 < x < 4$$, $$W(x) = 0$$. Our bounds are 2 to x, as the section starts at 2m, and we are cutting it to an arbritary 'x' m as we want the general equation for the shear force, not the exact shear force at the location. 
>
> > $$ V_{2} = -80 - \int_{2}^{x} 0 \,dx \rightarrow V_{2} = -80 $$
>
> $$ M_{2} = M_{A} + \int_{2}^{x} V(x) \,dx $$
>
There is no external moment present at the beginning of this section, so our $$M_A$$ is just 0. For $$V(x)$$, we just plug in the $$V(x)$$ we found previously.
>
> > $$ M_{2} = 0 + \int_{2}^{x} -80 \,dx \rightarrow M_{2} = -160 + 80x $$ 
